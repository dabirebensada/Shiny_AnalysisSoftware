    ...
    tabItems(
      ...
      tabItem(tabName = "help",
              fluidPage(
                ...
                p("Variables cibles : doivent être numériques (0, 1, 2, ...), integer ou réel (pour une classification elles doivent être impérativement en integer).Le logiciel prend en compte la dernière colonne comme variable cible, donc en fonction du nombre de variables cibles choisies, veuillez vous assurez que cela correspond dans votre base de donnée et est au bon format."),
                ...
                p("Si vous choisissez de lancer l'analyse des réseaux de neurones seule ou avec d'autres analyses, que vous soyez en classification ou en régréssion , vous devrez obligatoirement cliquer sur le boutton ajuster les paramètres apres avoir choisis vos modèles puis enregistrer sans quoi le lancement de fonctionnera pas"),
                p("Sachez aussi qu'il n'y a pas de graphiques pour l'analyse des réseaux de neurones et de la régréssion linéaire"),
                p("Sur certaines Analyses tels que les réseaux de neurones et ne Random Forest, lors de l'exécution des analyses il peut y avoir des notifications d'erreur qui vont apparaitre, mais ne vous en faites pas cela est normal et n'affecte en aucun cas vos résultats"),
                p("Il se peut que sur la version Web du logicel, la fonction d'envoi par email ne fonctionne pas. Cela est du a des soucis servers lié a la capacité octroyée au logiciel. Si vous tenez a tester intégralement le logiciel, vous pourrez le faire en recupérant tous les fichiers sur le GitHub <https://github.com/dabirebensada/Shiny_AnalysisSoftware.git>, et exécuter l'ensemble en local sur Rstudo. "),
                br(),
        ...
        if (model == "Random Forest") {
          ...
          if (input$analysis_type == "Classification") {
            ...
            output$regression_plot <- renderPlot({
              ggplot() +
                geom_point(aes(x = train_targets, y = train_pred), color = "blue", alpha = 0.5) +
                geom_abline(intercept = 0, slope = 1, color = "red") +
                labs(title = "Train Data: Predicted vs Actual", x = "Actual", y = "Predicted")
            })
          }
          ...
          # Generate the plot for linear regression
          output$linreg_plot <- renderPlot({
            ggplot() +
              geom_point(aes(x = train_targets, y = train_pred), color = "blue", alpha = 0.5) +
              geom_abline(intercept = 0, slope = 1, color = "red") +
              labs(title = "Train Data: Predicted vs Actual", x = "Actual", y = "Predicted")
